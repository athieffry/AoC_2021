...
# b. create initial matrix
    # empty matrix of max dimensions
    max_x <- max(df$x)
    max_y <- max(df$y)
...
mat[mat > 1] <- 1
pheatmap::pheatmap(mat2, cluster_rows=F, cluster_cols=F, cellwidth=5, cellheight=5, color=c('white', 'black'), legend=F)
